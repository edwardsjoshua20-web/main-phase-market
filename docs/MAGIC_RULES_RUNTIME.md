...
Four narrow resolver defects were found and fixed in canonical owners. Exact card names containing rules words no longer manufacture combat intent (`Fire Nation Attacks`); equivalent player-priority phrases such as `Priority is mine` and `I hold priority` populate the same canonical fact; a nearby opponent turn reference no longer makes the opponent the actor of a passive player permission question; and a trailing conjunction no longer contaminates the preceding exact card-name candidate in multi-card sentences. Explicit opponent action clauses remain opponent-owned, and conjunctions inside card names remain intact. Dedicated Phase 10 regressions cover every repair.
