...
const DEFAULT_SOURCE_PATH = path.resolve(process.cwd(), 'public/data/yugioh/cards.json');
const IMAGE_ROOT = path.resolve(process.cwd(), 'public/data/yugioh/images');
const DEFAULT_KINDS = ['small', 'full'];
const KIND_MAP = {
  small: 'image_url_small',
  full: 'image_url',
  cropped: 'image_url_cropped'
};
const CONCURRENCY = Number(process.env.YUGIOH_IMAGE_CONCURRENCY || 8);
...
async function downloadFile(url, destinationPath) {
  const response = await fetch(url, {
    headers: {
      'User-Agent': 'MainPhaseMarket/1.0 yugioh image mirror'
    }
  });
...
async function main() {
  ...
  if (firstArg === '--help' || firstArg === '-h') {
    console.log('Usage: node scripts/mirror-yugioh-images.mjs [small,full,cropped] [sourcePath]');
    process.exit(0);
  }

  const requestedKinds = (firstArg || process.env.YUGIOH_IMAGE_KINDS || DEFAULT_KINDS.join(','))
    .split(',')
  ...
  const sourcePath = process.env.YUGIOH_SOURCE_PATH || process.argv[3] || DEFAULT_SOURCE_PATH;
  if (!fs.existsSync(sourcePath)) {
    throw new Error(`Yu-Gi-Oh source file not found: ${sourcePath}`);
  }
  ...
  for (const card of Array.isArray(cards) ? cards : []) {
    const firstImage = Array.isArray(card?.card_images) ? card.card_images[0] : null;
    const cardId = firstImage?.id || card?.id;
    if (!cardId || uniqueById.has(cardId)) continue;
    uniqueById.set(cardId, {
      id: cardId,
      name: card?.name || '',
      images: {
        small: firstImage?.image_url_small || null,
        full: firstImage?.image_url || null,
        cropped: firstImage?.image_url_cropped || null
      }
    });
  }
      ...
      tasks.push(async () => {
        try {
          ...
          if (stats.downloaded % 1000 === 0) {
            console.log(`Downloaded ${stats.downloaded}/${stats.downloadsQueued} Yu-Gi-Oh images...`);
          }
        } catch (error) {
          stats.failed += 1;
          console.error(`Yu-Gi-Oh image download failed for ${row.name} [${row.id}] ${kind}: ${error.message}`);
        }
      });
    }
  }

  console.log(`Found ${stats.cardsSeen} unique Yu-Gi-Oh cards.`);
  ...
  console.log('Yu-Gi-Oh image mirror complete.');
  console.log(JSON.stringify(manifest, null, 2));
}

main().catch((error) => {
  console.error('Yu-Gi-Oh image mirror failed:', error);
  process.exitCode = 1;
});
