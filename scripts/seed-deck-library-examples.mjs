...
function itemFromCard(game, card) {
  ...
  if (game === 'onepiece') return { product_id: `onepiece:${card.id}`, api_id: card.id, product_name: card.name, product_image: card.image_url || '', image_url: card.image_url || '', product_type: game, game, type: card.category || '', type_line: card.category || '', set_code: card.set_code || '', rarity: card.rarity || '', quantity: 1 };
