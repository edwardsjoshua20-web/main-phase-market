...
function scoreStarWarsCard(card, normalizedQuery) {
  ...
  if (searchTextEquals(`${name} ${subtitle}`.trim(), normalizedQuery)) return 950;
  ...
  if (searchTextIncludes(subtitle, normalizedQuery)) return 250;
  if (searchTextIncludes(text, normalizedQuery)) return 180;
  return 0;
}
